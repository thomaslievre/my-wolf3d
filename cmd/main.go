...
func render(screen *ebiten.Image) {
	...
	for castColumn = 0; castColumn < PROJECTIONPLANEWIDTH; castColumn += 1 {
		...
		var dist float64
		var topOfWall int    // used to compute the top and bottom of the sliver that
		var bottomOfWall int // will be the staring point of floor and ceiling
		var renderedColor color.RGBA
		...
		bottomOfWall = fProjectionPlaneYCenter + int(projectedWallHeight*0.5)
